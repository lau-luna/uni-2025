...
public class NumberUtils {
    ...
    public ArrayList<Integer> divisores(int n) {
        ArrayList<Integer> divs = new ArrayList<>(2);
        for (int i = 1; i <= n; i++) {
            if (n % i == 0)
                divs.add(i);
        }
    ...
    public int nthFib(int n) {
        assert n > 0;
        int[] fib = ArrayUtils.fibSequence(n);
    ...
    public ArrayList<Integer> digitos(int n) {
        String num = "" + n;
        ArrayList<Integer> digits = new ArrayList<>(num.length());
    ...
    public float minimo(float[] arreglo) {
        assert arreglo != null : "null array.";
        assert arreglo.length > 0 : "empty array";
        float min = arreglo[0];
